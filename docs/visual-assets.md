...
Caption authority: `naca0012-airfoil/README.md` (§1 to 12) and
...
| `naca0012-airfoil/09-near-wall-velocity-laws.png` | u⁺ vs y⁺, viscous law and log law, viscous sublayer and the 30 < y⁺ < 300 standard wall-function target band | Meshing / near-wall | `full` |
...
| `naca0012-airfoil/10-experimental-cp-reference.png` | **Experimental upper-surface Cp reference data only** (NASA), inverted axis. *Verified by inspection: this is the reference dataset, NOT a CFD-vs-experiment overlay, so the caption must not imply otherwise.* | Validation | `pair` R |
...
`naca0012-airfoil/07-mesh-strategy.png`. It is a labelled sketch rather than
solver output, and the same four refinement decisions read better as text.
The Meshing section presents them in a `specGrid` block instead.

Everything else tracked in this repository is technically meaningful and
...
| `figures/pm/gantt-design-build-phase.png` | Design and build phase Gantt (weeks 7 to 17) | Contribution / PM | `pair` R |
...
- `figures/structures/subassembly-mass-table.png`,
  ...
  are raw table screenshots, superseded by the generated charts.
